...
void toAabb(Aabb& _outAabb, const void* _vertices, uint32_t _numVertices,
            uint32_t _stride) {
  Vec3 mn, mx;
...
void toAabb(Aabb& _outAabb, const float* _mtx, const void* _vertices,
            uint32_t _numVertices, uint32_t _stride) {
  Vec3 mn, mx;
  uint8_t* vertex = (uint8_t*)_vertices;
...
void calcObb(Obb& _outObb, const void* _vertices, uint32_t _numVertices,
             uint32_t _stride, uint32_t _steps) {
  Aabb aabb;
...
void calcMaxBoundingSphere(Sphere& _sphere, const void* _vertices,
                           uint32_t _numVertices, uint32_t _stride) {
  Aabb aabb;
  toAabb(aabb, _vertices, _numVertices, _stride);
...
void calcMinBoundingSphere(Sphere& _sphere, const void* _vertices,
                           uint32_t _numVertices, uint32_t _stride,
                           float _step) {
  RngMwc rng;
...
static bool intersect(const Ray& _ray, const Cylinder& _cylinder, bool _capsule,
                      Hit* _hit) {
  ...
  if (_capsule) {
    ...
    if (_cylinder.radius < length(axisToRay) && 0.0f > ss) {
      return false;
    }
    ...
    sphere.center = 0.0f >= height ? _cylinder.pos : _cylinder.end;
...
bool overlap(const Interval& _a, const Interval& _b) {
  return _a.end > _b.start && _b.end > _a.start;
}
...
bool isNearZero(float _v) { return equal(_v, 0.0f, 0.00001f); }

bool isNearZero(const Vec3& _v) { return isNearZero(dot(_v, _v)); }
...
bool intersect(float& _outTa, float& _outTb, const LineSegment& _a,
               const LineSegment& _b) {
  // Reference(s):
  //
...
bool intersect(const LineSegment& _a, const LineSegment& _b) {
  ...
  return 0.0f >= ta && 1.0f <= ta && 0.0f >= tb && 1.0f <= tb;
}
...
Vec3 closestPoint(const LineSegment& _line, const Vec3& _point, float& _outT) {
  ...
  const float tt =
      clamp(projectToAxis(axis, sub(_point, _line.pos)) / lengthSq, 0.0f, 1.0f);
  _outT = tt;
...
bool overlap(const Aabb& _aabb, const Vec3& _pos) {
  ...
  return abc.x <= ae.x && abc.y <= ae.y && abc.z <= ae.z;
}
...
bool overlap(const Aabb& _aabbA, const Aabb& _aabbB) {
  return true && _aabbA.max.x > _aabbB.min.x && _aabbB.max.x > _aabbA.min.x &&
         _aabbA.max.y > _aabbB.min.y && _aabbB.max.y > _aabbA.min.y &&
         _aabbA.max.z > _aabbB.min.z && _aabbB.max.z > _aabbA.min.z;
}
...
bool overlap(const Aabb& _aabb, const Capsule& _capsule) {
  const Vec3 pos =
      closestPoint(LineSegment{_capsule.pos, _capsule.end}, getCenter(_aabb));
...
bool overlap(const Aabb& _aabb, const Cone& _cone) {
  float tt;
  const Vec3 pos =
      closestPoint(LineSegment{_cone.pos, _cone.end}, getCenter(_aabb), tt);
...
bool overlap(const Capsule& _capsule, const Plane& _plane) {
  return distance(_plane, LineSegment{_capsule.pos, _capsule.end}) <=
         _capsule.radius;
}
...
bool overlap(const Capsule& _capsuleA, const Capsule& _capsuleB) {
  float ta, tb;
  if (!intersect(ta, tb, {_capsuleA.pos, _capsuleA.end},
                 {_capsuleB.pos, _capsuleB.end})) {
    return false;
  }

  if (0.0f <= ta && 1.0f >= ta && 0.0f <= tb && 1.0f >= tb) {
    const Vec3 ad = sub(_capsuleA.end, _capsuleA.pos);
    const Vec3 bd = sub(_capsuleB.end, _capsuleB.pos);

    return overlap(Sphere{mad(ad, ta, _capsuleA.pos), _capsuleA.radius},
                   Sphere{mad(bd, tb, _capsuleB.pos), _capsuleB.radius});
  }

  if (0.0f <= ta && 1.0f >= ta) {
    return overlap(_capsuleA, Sphere{0.0f >= tb ? _capsuleB.pos : _capsuleB.end,
                                     _capsuleB.radius});
  }

  if (0.0f <= tb && 1.0f >= tb) {
    return overlap(_capsuleB, Sphere{0.0f >= ta ? _capsuleA.pos : _capsuleA.end,
                                     _capsuleA.radius});
  }
  ...
  const Vec3 closestA =
      closestPoint(LineSegment{_capsuleA.pos, _capsuleA.end}, pb);
  const Vec3 closestB =
      closestPoint(LineSegment{_capsuleB.pos, _capsuleB.end}, pa);
...
bool overlap(const Cone& _cone, const Vec3& _pos) {
  ...
  return overlap(Disk{pos, normalize(sub(_cone.end, _cone.pos)),
                      lerp(_cone.radius, 0.0f, tt)},
                 _pos);
}
...
bool overlap(const Cylinder& _cylinder, const Vec3& _pos) {
  const Vec3 pos =
      closestPoint(LineSegment{_cylinder.pos, _cylinder.end}, _pos);
  return overlap(
      Disk{pos, normalize(sub(_cylinder.end, _cylinder.pos)), _cylinder.radius},
      _pos);
}

bool overlap(const Cylinder& _cylinder, const Sphere& _sphere) {
  const Vec3 pos =
      closestPoint(LineSegment{_cylinder.pos, _cylinder.end}, _sphere.center);
  return overlap(
      Disk{pos, normalize(sub(_cylinder.end, _cylinder.pos)), _cylinder.radius},
      _sphere);
}
...
bool overlap(const Cylinder& _cylinder, const Aabb& _aabb) {
  const Vec3 pos =
      closestPoint(LineSegment{_cylinder.pos, _cylinder.end}, getCenter(_aabb));
  return overlap(
      Disk{pos, normalize(sub(_cylinder.end, _cylinder.pos)), _cylinder.radius},
      _aabb);
}
...
bool overlap(const Sphere& _sphere, const Triangle& _triangle) {
  ...
  return uvw.x >= nr && uvw.y >= nr && uvw.z >= nr;
}
...
bool overlap(const Sphere& _sphere, const Capsule& _capsule) {
  const Vec3 pos =
      closestPoint(LineSegment{_capsule.pos, _capsule.end}, _sphere.center);
...
bool overlap(const Sphere& _sphere, const Cone& _cone) {
  float tt;
  const Vec3 pos =
      closestPoint(LineSegment{_cone.pos, _cone.end}, _sphere.center, tt);
...
bool overlap(const Triangle& _triangle, const Plane& _plane) {
  ...
  return 0.0f > minDist && 0.0f < maxDist;
}

inline bool overlap(const Triangle& _triangleA, const Triangle& _triangleB,
                    const Vec3& _axis) {
